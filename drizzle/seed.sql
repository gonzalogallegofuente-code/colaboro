...
INSERT INTO kids (name, emoji, color, sort_order)
...
INSERT INTO tasks (name, description, icon, value_cents, weekly_target, color, sort_order)
SELECT v.name, v.description, v.icon, v.value_cents, v.weekly_target, v.color, v.sort_order
FROM (VALUES
  ...
  ('Recoger lavaplatos',  'sacar y colocar',                                      '🍽️', 100, 7, '#e3e3c5', 9),
  ('Tirar la basura',     'orgánico, plásticos, papel y vidrio',                   '🗑️',  50, 7, '#c5cfe2', 10)
) AS v(name, description, icon, value_cents, weekly_target, color, sort_order)
